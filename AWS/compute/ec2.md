...
## EC2 타입 및 용량 변경
블루/그린 배포를 통해 다운타임을 없앨 수 있다. 
1. 기존 인스턴스의 이미지(AMI)를 생성한다. 
2. 해당 이미지를 이용하여 타입이 변경된 새로운 인스턴스를 만든다. (중지 상태에서 가능)
3. 새 인스턴스를 active 상태로 전환한다.
4. 기존 인스턴스와 연결되어 있던 EIP(Elastic IP)를 새 ec2 인스턴스와 attach해준다.  
5. 이후 기존 인스턴스가 트래픽을 수신하지 않으면 이전 인스턴스를 종료한다. 